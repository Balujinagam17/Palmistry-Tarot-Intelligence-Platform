...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const userName = name || email.split("@")[0] || "Aetheria Seeker";
    onLoginSuccess(userName, email || "seeker@aetheria.ai");
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
      ...
      >
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-slate-400 hover:text-white p-2 rounded-xl bg-slate-900 border border-slate-800 transition-all cursor-pointer"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full py-3.5 rounded-xl bg-gradient-to-r from-cyan-500 via-blue-600 to-purple-600 hover:from-cyan-400 hover:to-purple-500 text-slate-950 font-black text-xs font-mono uppercase tracking-wider shadow-[0_0_20px_rgba(6,182,212,0.4)] transition-all flex items-center justify-center gap-2 cursor-pointer mt-2"
          >
            <span>
              {mode === "login"
                ? "Sign In"
